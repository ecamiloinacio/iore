...
#include <stdlib.h>
#include <stdio.h>
/* TODO: check if lustreapi.h is available */
...
int
proto_create (iore_file_t *, const iore_test_t *);
int
proto_open (iore_file_t *, const iore_test_t *);
ssize_t
proto_write_oset (iore_file_t, const void *, const off_t *, const iore_test_t *);
ssize_t
proto_read_oset (iore_file_t, void *, const off_t *, const iore_test_t *);
ssize_t
proto_write_dset (iore_file_t, const void *, const iore_test_t *);
ssize_t
proto_read_dset (iore_file_t, void *, const iore_test_t *);
int
proto_close (iore_file_t *);
int
proto_remove (iore_file_t);

/*** VARIABLES ***************************************************************/

const iore_afio_vtable_t afio_proto =
  { proto_create, proto_open, proto_write_oset, proto_read_oset,
      proto_write_dset, proto_read_dset, proto_close, proto_remove };

/*** FUNCTIONS ***************************************************************/

int
proto_create (iore_file_t *file, const iore_test_t *test)
{
  ...
  if (test->file_mode == IORE_TEST_FMODE_NXN)
    {
      ...
      llapi_get_obd_count (mnt, &nosts, 0);
...
int
proto_open (iore_file_t *file, const iore_test_t *test)
{
  int rerr = IORE_SUCCESS;
...
ssize_t
proto_write_oset (iore_file_t file, const void *buf, const off_t *offs,
...
ssize_t
proto_read_oset (iore_file_t file, void *buf, const off_t *offs,
		 const iore_test_t *test)
{
  assert(buf);
  assert(offs);
...
ssize_t
proto_write_dset (iore_file_t file, const void *buf, const iore_test_t *test)
{
  assert(buf);
  assert(test);
...
ssize_t
proto_read_dset (iore_file_t file, void *buf, const iore_test_t *test)
{
  assert(buf);
  assert(test);
